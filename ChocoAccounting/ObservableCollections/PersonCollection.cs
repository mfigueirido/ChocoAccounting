...
namespace ChocoAccounting.ObservableCollections
{
    public class PersonCollection
        : ExpandedCollection<Person>
    {
        public PersonCollection(IEnumerable<Person> persons, MovementsModel context)
            : base(persons, context)
        {
            Context = context;
        }
        ...
        {
            Context.PersonSet.Add(item);
            base.InsertItem(index, item);
        }

        protected override void RemoveItem(int index)
        {
            Context.PersonSet.Remove(this[index]);
